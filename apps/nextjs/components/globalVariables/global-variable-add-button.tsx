...
export function GlobalVariableAddButton({ ...props }: GlobalVariableAddButtonProps) {
  ...
  async function onClick() {
    ...
    if (!response?.ok) {
      ...
      return toast({
        ...
        description: "Your globalVariable was not created. Please try again.",
        variant: "destructive",
      })
    }
    ...
    const globalVariable = await response.json()

    setIsLoading(false)
    setShowAddAlert(false)

    router.push(`/dashboard/globalVariables/${globalVariable.id}/settings`)
    router.refresh()
  }
