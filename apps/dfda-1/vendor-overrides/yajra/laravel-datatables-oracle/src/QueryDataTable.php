...
class QueryDataTable extends DataTableAbstract
{
    ...
    public function make($mDataSupport = true)
    {
        try {
            ...
            $res = $this->render($data);
            $this->saveDebugInfoToMemory([
    ...
    public function saveDebugInfoToMemory(array $output){
        $data = $this->showDebugger($output);
	    TestMemory::set(Memory::DATATABLE_RESPONSE, $data);
    }

    public static function getDebugInfo(): array {
        return TestMemory::get(Memory::DATATABLE_RESPONSE) ?? [];
    }

    public static function getQueries(): array {
        $data = self::getDebugInfo();
		if(!$data){
			return [];
		}
        $queries = QMArr::pluckColumn($data['queries'], "query");
        return $queries;
    }
