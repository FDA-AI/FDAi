...
class DatalabAggregateCorrelationsTest extends DatalabTestCase
{
	...
	public function testDatalabAggregateCorrelationsAsRegularUser(): void{
        $this->actAsTestUser();
    ...
    public function testDatalabAggregateCorrelationsAsAdmin(): void{
        $this->actAsAdmin();
        $response = $this->stagingRequest(200, "");
        $this->assertCount(10, $this->lastResponseData('data'));
        $this->assertDataTableQueriesEqual(array (
            ...
            2 => 'select count(*) as aggregate from (select * from `aggregate_correlations` where `aggregate_correlations`.`cause_variable_id` = ? and `aggregate_correlations`.`deleted_at` is null order by `aggregate_correlations`.`aggregate_qm_score` desc) count_row_table',
            3 => 'select * from `aggregate_correlations` where `aggregate_correlations`.`cause_variable_id` = ? and `aggregate_correlations`.`deleted_at` is null order by `aggregate_correlations`.`aggregate_qm_score` desc limit 10 offset 0',
  ...
  6 => 'select * from `votes` where `votes`.`aggregate_correlation_id` in (65689914, 65691033, 65692817, 65694448, 65696617, 65702433, 65702437, 65704480, 65706008, 65756242) and `votes`.`deleted_at` is null',
        ));
        $this->checkTestDuration(20);
        $this->checkQueryCount(12);
    }
    public function testDatalabAggregateCorrelationsAsWithoutAuth(): void{
