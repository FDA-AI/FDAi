...
class AggregateCorrelationApiTest extends UnitTestCase
{
	 ...
	 * @return void
	 * @throws \Throwable
	 * @covers \App\Http\Controllers\API\AggregateCorrelationAPIController::index
	 */
    public function test_read_aggregate_correlations(): void{
	    $this->checkUserTable();
	    $this->checkMeasurements();
	    $ac = AggregateCorrelation::first();
        if(!$ac) {
            $this->fail("No aggregate correlations found");
        }
        ...
        $r = $this->jsonAsUser18535(
            'GET',
            '/api/v6/aggregate_correlations/'.$id
        );
        $r->assertStatus(200);
        $data = $this->getJsonResponseData();
        $this->compareObjectFixture('aggregate_correlations', $data);
        $ac = AggregateCorrelation::fakeSaveFromPropertyModels();
        $r = $this->jsonAsUser18535(
            'GET',
            '/api/v6/aggregate_correlations'
        );
        $r->assertStatus(200);
        $data = $this->getJsonResponseData();
        //var_export($data);
        $this->assertEquals(array (
            ...
            return \'People with an average of \' + this.x +
                ...
                this.y + \'/5 Overall Mood\';
			                            ...
			                            'variables_where_best_aggregate_correlation_count' => NULL,
			                            'vote' => NULL,
			                            'votes_count' => NULL,
			                            'wp_post_id' => NULL,
			                            'z_score' => 1.06,
        ...
        $r = $this->jsonAsUser18535(
            'PUT',
            '/api/v6/aggregate_correlations/'.$ac->id,
            [AggregateCorrelation::FIELD_PREDICTS_LOW_EFFECT_CHANGE => 1]
        );
        $r->assertStatus(401);
        $this->expectUnauthorizedException();
        $r = $this->jsonAsUser18535(
