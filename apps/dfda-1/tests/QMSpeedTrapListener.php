 ...
 */
class QMSpeedTrapListener implements TestListener
{
    ...
    {
        $this->enabled = !(\App\Utils\Env::get('PHPUNIT_SPEEDTRAP') === 'disabled');
    ...
    protected function renderBody()
    {
        ...
        for ($i = 1; $i <= $length; ++$i) {
	        ...
	        /** @var QMBaseTestCase $test */
	        $test = $this->tests[$label];
			if(method_exists($test, 'getProfileUrl')) {
				$url = $test->getProfileUrl();
				if(!$url){$url  = $test->getPHPStormUrl();}
			} elseif($test instanceof JobTestCase) {
				$url = $test->getPHPStormUrl();
			} else {
                $url = ClockworkButton::url();
            }
	        $str = sprintf(" %s. %sms to run %s %s\n", $i, $time, $label, $url);
            echo $str;
        }
    }
