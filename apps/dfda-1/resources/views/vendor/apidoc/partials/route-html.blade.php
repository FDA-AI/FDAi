<!-- START_{{$route['id']}} -->
<div id="{{$route['id']}}" class="route-block">
    <div class="route-info-column">
        @if($route['metadata']['title'] != '')
            <h2 class="route-title">{!! $route['metadata']['title'] !!}</h2>
        @else
            <h2 class="route-title">{{$route['uri']}}</h2>
        @endif

        @if($route['metadata']['authenticated'])
            <br><small style="padding: 1px 9px 2px;font-weight: bold;white-space: nowrap;color: #ffffff;-webkit-border-radius: 9px;-moz-border-radius: 9px;border-radius: 9px;background-color: #3a87ad;">
                Requires authentication</small>
        @endif
