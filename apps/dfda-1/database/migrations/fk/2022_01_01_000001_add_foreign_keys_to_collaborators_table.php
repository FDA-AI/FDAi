...
class AddForeignKeysToCollaboratorsTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('collaborators', function (Blueprint $table) {
            $table->foreign(['app_id'], 'collaborators_applications_id_fk')->references(['id'])->deferrable()->on('applications')->onUpdate('CASCADE')->onDelete('CASCADE');
            $table->foreign(['client_id'], 'collaborators_client_id_fk')->references(['client_id'])->on('oa_clients')->onUpdate('CASCADE')->onDelete('CASCADE');
            $table->foreign(['user_id'], 'collaborators_user_id_fk')->references(['ID'])->deferrable()->on('wp_users')->onUpdate('CASCADE')->onDelete('CASCADE');
        });
    }
