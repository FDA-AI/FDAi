...
class AddForeignKeysToOaRefreshTokensTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('oa_refresh_tokens', function (Blueprint $table) {
            $table->foreign(['client_id'], 'bshaffer_oauth_refresh_tokens_client_id_fk')->references(['client_id'])->on('oa_clients');
            $table->foreign(['user_id'], 'bshaffer_oauth_refresh_tokens_user_id_fk')->references(['ID'])->deferrable()->on('wp_users');
