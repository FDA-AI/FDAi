...
class CreateWpPostsTable extends Migration
{
    ...
    public function up()
    {
         Schema::create('wp_posts', function (Blueprint $table) {
            ...
            $table->bigInteger('post_author')->nullable()->default(0)->index('post_author')->comment('The user ID who created it.');
            $table->timestamp('post_date')->nullable();
            $table->timestamp('post_date_gmt')->nullable();
            $table->text('post_content')->nullable()->comment('Holds all the content for the post, including HTML, shortcodes and other content.');
            ...
            $table->timestamp('post_modified')->nullable()->index();
            $table->timestamp('post_modified_gmt')->nullable();
            $table->text('post_content_filtered')->nullable()->comment('Used by plugins to cache a version of post_content typically passed through the ‘the_content’ filter. Not used by WordPress core itself.');
            $table->bigInteger('post_parent')->nullable()->default(0)->index('post_parent')->comment('Used to create a relationship between this post and another when this post is a revision, attachment or another type.');
