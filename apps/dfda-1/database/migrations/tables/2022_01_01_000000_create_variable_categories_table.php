...
class CreateVariableCategoriesTable extends Migration
{
    ...
    public function up()
    {
         Schema::create('variable_categories', function (Blueprint $table) {
            ...
            $table->float('filling_value', 0, 0)->nullable()->comment('Value for replacing null measurements');
            $table->float('maximum_allowed_value', 0, 0)->nullable()->comment('Maximum recorded value of this category');
            $table->float('minimum_allowed_value', 0, 0)->nullable()->comment('Minimum recorded value of this category');
            $table->integer('duration_of_action')->default(86400)->comment('How long the effect of a measurement in this variable lasts');
            $table->integer('onset_delay')->default(0)->comment('How long it takes for a measurement in this variable to take effect');
            ...
            $table->smallInteger('default_unit_id')->nullable()->default(12)->index('variable_categories_default_unit_id_fk')->comment('ID of the default unit for the category');
            $table->softDeletes();
            $table->boolean('manual_tracking')->nullable()->default(false)->comment('Should we include in manual tracking searches?');
            ...
            $table->bigInteger('wp_post_id')->nullable()->index('variable_categories_wp_posts_ID_fk');
            $table->enum('filling_type', ['zero', 'none', 'interpolation', 'value'])->nullable();
            $table->integer('number_of_outcome_population_studies')->nullable()->comment('Number of Global Population Studies for this Cause Variable Category.
                [Formula: 
                ...
                ');
            $table->integer('number_of_predictor_population_studies')->nullable()->comment('Number of Global Population Studies for this Effect Variable Category.
                [Formula: 
                    update variable_categories
                        left join (
                ...
                ');
            $table->integer('number_of_outcome_case_studies')->nullable()->comment('Number of Individual Case Studies for this Cause Variable Category.
                [Formula: 
                    update variable_categories
                        left join (
                            select count(id) as total, cause_variable_category_id
                ...
                ');
            $table->integer('number_of_predictor_case_studies')->nullable()->comment('Number of Individual Case Studies for this Effect Variable Category.
                [Formula: 
                    update variable_categories
                        left join (
                            select count(id) as total, effect_variable_category_id
                ...
                ');
            $table->integer('number_of_measurements')->nullable()->comment('Number of Measurements for this Variable Category.
            ...
            $table->integer('number_of_user_variables')->nullable()->comment('Number of User Variables for this Variable Category.
                    [Formula: update variable_categories
                        left join (
                            select count(id) as total, variable_category_id
                            from user_variables
            ...
            $table->integer('number_of_variables')->nullable()->comment('Number of Variables for this Variable Category.
                    [Formula: update variable_categories
                        left join (
                            select count(id) as total, variable_category_id
                            from variables
