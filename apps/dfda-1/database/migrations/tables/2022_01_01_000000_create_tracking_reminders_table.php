...
class CreateTrackingRemindersTable extends Migration
{
    ...
    public function up()
    {
         Schema::create('tracking_reminders', function (Blueprint $table) {
            ...
            $table->integer('variable_id')->comment('Id for the variable to be tracked');
            $table->float('default_value', 0, 0)->nullable()->comment('Default value to use for the measurement when tracking');
            ...
            $table->integer('user_variable_id')->index('tracking_reminders_user_variables_user_variable_id_fk');
            $table->timestamp('latest_tracking_reminder_notification_notify_at')->nullable();
            $table->integer('number_of_tracking_reminder_notifications')->nullable()->comment('Number of Tracking Reminder Notifications for this Tracking Reminder.
