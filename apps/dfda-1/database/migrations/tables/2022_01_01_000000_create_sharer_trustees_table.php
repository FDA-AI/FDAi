...
class CreateSharerTrusteesTable extends Migration
{
    ...
    public function up()
    {
         Schema::create('sharer_trustees', function (Blueprint $table) {
            $table->increments('id');
            $table->bigInteger('sharer_user_id')->comment('The sharer who has granted data access to the trustee. ');
            $table->bigInteger('trustee_user_id')->index('sharer_trustees_wp_users_ID_fk_2')->comment('The trustee who has been granted access to the sharer data.');
