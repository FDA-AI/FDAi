...
class CreateCommonTagsTable extends Migration
{
    ...
    public function up()
    {
         Schema::create('common_tags', function (Blueprint $table) {
            ...
            $table->integer('tag_variable_id')->index('common_tags_tag_variable_id_variables_id_fk')->comment('This is the id of the ingredient variable whose value is determined based on the value of the tagged variable.');
            $table->integer('number_of_data_points')->nullable()->comment('The number of data points used to estimate the mean. ');
            $table->float('standard_error', 0, 0)->nullable()->comment('Measure of variability of the 
mean value as a function of the number of data points.');
            $table->smallInteger('tag_variable_unit_id')->nullable()->index('common_tags_tag_variable_unit_id_fk')->comment('The id for the unit of the tag (ingredient) variable.');
            $table->smallInteger('tagged_variable_unit_id')->nullable()->index('common_tags_tagged_variable_unit_id_fk')->comment('The unit for the source variable to be tagged.');
