...
class CreateCorrelationsTable extends Migration
{
    ...
    public function up()
    {
         Schema::create('user_variable_relationships', function (Blueprint $table) {
            ...
            $table->integer('effect_variable_id')->index('correlations_effect_variable_id_fk');
            $table->float('qm_score', 0, 0)->nullable()->comment('A number representative of the relative importance of the relationship based on the strength, 
            ...
            $table->float('forward_pearson_correlation_coefficient', 0, 0)->nullable()->comment('Pearson correlation coefficient between cause and effect measurements');
            $table->float('value_predicting_high_outcome', 0, 0)->nullable()->comment('cause value that predicts an above average effect value (in default unit for cause variable)');
            $table->float('value_predicting_low_outcome', 0, 0)->nullable()->comment('cause value that predicts a below average effect value (in default unit for cause variable)');
            ...
            $table->smallInteger('cause_unit_id')->nullable()->index('correlations_cause_unit_id_fk')->comment('Unit ID of Cause');
            ...
            $table->float('forward_spearman_correlation_coefficient', 0, 0)->comment('Predictive spearman correlation of the lagged pair data. While the Pearson correlation assesses linear relationships, the Spearman correlation assesses monotonic relationships (whether linear or not).');
            ...
            $table->float('optimal_pearson_product', 0, 0)->nullable()->comment('Optimal Pearson Product');
            ...
            $table->bigInteger('wp_post_id')->nullable()->index('correlations_wp_posts_ID_fk');
            $table->string('status', 25)->nullable();
            $table->smallInteger('cause_variable_category_id')->index('correlations_cause_variable_category_id_fk');
            $table->smallInteger('effect_variable_category_id')->index('c_effect_variable_category_id_fk');
            ...
            $table->integer('cause_user_variable_id')->index('correlations_user_variables_cause_user_variable_id_fk');
            $table->integer('effect_user_variable_id')->index('correlations_user_variables_effect_user_variable_id_fk');
            ...
            $table->float('effect_baseline_average', 0, 0)->nullable()->comment('Outcome Average at Baseline (The normal value for the outcome seen without treatment during the previous duration of action time span)');
            ...
            $table->float('effect_baseline_standard_deviation', 0, 0)->nullable()->comment('Outcome Relative Standard Deviation at Baseline (How much the outcome value normally fluctuates without treatment during the previous duration of action time span)');
            ...
            $table->integer('number_of_up_votes')->nullable()->comment('Number of people who feel this relationship is plausible and useful. ');
            $table->integer('number_of_down_votes')->nullable()->comment('Number of people who feel this relationship is implausible or not useful. ');
            ...
            $table->unique(['user_id', 'cause_variable_id', 'effect_variable_id'], 'correlations_user_id_cause_variable_id_effect_variable_id_uinde');
        });
    }
