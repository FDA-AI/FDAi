...
class CreateCorrelationCausalityVotesTable extends Migration
{
    ...
    public function up()
    {
         Schema::create('correlation_causality_votes', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('cause_variable_id')->index('correlation_causality_votes_cause_variables_id_fk');
            $table->integer('effect_variable_id')->index('correlation_causality_votes_effect_variables_id_fk');
