...
class CreateChildParentsTable extends Migration
{
    ...
    public function up()
    {
         Schema::create('child_parents', function (Blueprint $table) {
            $table->increments('id');
            $table->bigInteger('child_user_id')->comment('The child who has granted data access to the parent. ');
            $table->bigInteger('parent_user_id')->index('child_parents_wp_users_ID_fk_2')->comment('The parent who has been granted access to the child data.');
