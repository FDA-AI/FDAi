...
class CreateCohortStudyStatisticsTable extends Migration
{
    ...
    public function up()
    {
         Schema::create('cohort_study_statistics', function (Blueprint $table) {
            ...
            $table->float('forward_pearson_correlation_coefficient', 0, 0)->comment('Pearson correlation coefficient between Predictor and Outcome measurements');
            ...
            $table->float('value_predicting_high_outcome', 0, 0)->comment('cause value that predicts an above average Outcome value (in default unit for Predictor variable)');
            $table->float('value_predicting_low_outcome', 0, 0)->comment('cause value that predicts a below average Outcome value (in default unit for Predictor variable)');
            $table->float('optimal_pearson_product', 0, 0)->comment('Optimal Pearson Product');
            $table->float('average_vote', 0, 0)->nullable()->default(0.5)->comment('Vote');
            ...
            $table->smallInteger('cause_unit_id')->nullable()->index('cohort_study_statistics_cause_unit_id_fk')->comment('Unit ID of Predictor Variable');
            ...
            $table->float('Cohort_qm_score', 0, 0)->comment('A number representative of the relative importance of the relationship based on the strength, usefulness, and plausible causality.  The higher the number, the greater the perceived importance.  This value can be used for sorting relationships by importance. ');
            ...
            $table->float('average_effect_following_high_cause', 0, 0)->comment('The average Outcome variable measurement value following an above average Predictor value (in the common unit). ');
            $table->float('average_effect_following_low_cause', 0, 0)->comment('The average Outcome variable measurement value following a below average Predictor value (in the common unit). ');
            $table->float('average_daily_low_cause', 0, 0)->comment('The average of below average Predictor values (in the common unit). ');
            $table->float('average_daily_high_cause', 0, 0)->comment('The average of above average Predictor values (in the common unit). ');
            $table->float('population_trait_pearson_correlation_coefficient', 0, 0)->nullable()->comment('The pearson correlation of pairs which each consist of the average Predictor value and the average Outcome value for a given user. ');
            ...
            $table->smallInteger('cause_variable_category_id')->index('cohort_study_statistics_cause_variable_category_id_fk');
            $table->smallInteger('effect_variable_category_id')->index('cohort_study_statistics_effect_variable_category_id_fk');
            ...
            $table->boolean('is_public')->nullable()->comment('Indicates whether the study is private or should be publicly displayed.');
