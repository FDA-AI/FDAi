...
class CreateVariablesTable extends Migration
{
    ...
    public function up()
    {
         Schema::create('variables', function (Blueprint $table) {
            ...
            $table->smallInteger('variable_category_id')->comment('Variable category ID');
            $table->smallInteger('default_unit_id')->index('fk_variableDefaultUnit')->comment('ID of the default unit for the variable');
            $table->float('default_value', 0, 0)->nullable();
            ...
            $table->integer('duration_of_action')->nullable()->comment('How long the effect of a measurement in this variable lasts');
            ...
            $table->float('kurtosis', 0, 0)->nullable()->comment('Kurtosis');
            $table->float('maximum_allowed_value', 0, 0)->nullable()->comment('Maximum reasonable value for a single measurement for this variable in the default unit. ');
            $table->float('maximum_recorded_value', 0, 0)->nullable()->comment('Maximum recorded value of this variable');
            $table->float('mean', 0, 0)->nullable()->comment('Mean');
            ...
            $table->float('price', 0, 0)->nullable()->comment('Price');
            $table->string('product_url', 2083)->nullable()->comment('Product URL');
            $table->float('second_most_common_value', 0, 0)->nullable();
            $table->float('skewness', 0, 0)->nullable()->comment('Skewness');
            $table->float('standard_deviation', 0, 0)->nullable()->comment('Standard Deviation');
            $table->string('status', 25)->default('WAITING')->comment('status');
            $table->float('third_most_common_value', 0, 0)->nullable();
            $table->timestamp('updated_at')->useCurrent();
            $table->float('variance', 0, 0)->nullable()->comment('Variance');
            ...
            $table->integer('best_cause_variable_id')->nullable()->index('variables_best_cause_variable_id_fk');
            $table->integer('best_effect_variable_id')->nullable()->index('variables_best_effect_variable_id_fk');
            $table->float('common_maximum_allowed_daily_value', 0, 0)->nullable();
            $table->float('common_minimum_allowed_daily_value', 0, 0)->nullable();
            $table->float('common_minimum_allowed_non_zero_value', 0, 0)->nullable();
            ...
            $table->bigInteger('wp_post_id')->nullable()->index('variables_wp_posts_ID_fk');
            ...
            $table->integer('number_of_outcome_population_studies')->nullable()->comment('Number of Global Population Studies for this Cause Variable.
                ...
                ');
            $table->integer('number_of_predictor_population_studies')->nullable()->comment('Number of Global Population Studies for this Effect Variable.
                [Formula: 
                    update variables
                        left join (
                ...
                ');
            $table->integer('number_of_applications_where_outcome_variable')->nullable()->comment('Number of Applications for this Outcome Variable.
                [Formula: 
                    update variables
                        left join (
                ...
                ');
            $table->integer('number_of_applications_where_predictor_variable')->nullable()->comment('Number of Applications for this Predictor Variable.
                [Formula: 
                    update variables
                        left join (
                ...
                ');
            $table->integer('number_of_common_tags_where_tag_variable')->nullable()->comment('Number of Common Tags for this Tag Variable.
                [Formula: 
                    update variables
                        left join (
                ...
                ');
            $table->integer('number_of_common_tags_where_tagged_variable')->nullable()->comment('Number of Common Tags for this Tagged Variable.
                [Formula: 
                    update variables
                        left join (
                ...
                ');
            $table->integer('number_of_outcome_case_studies')->nullable()->comment('Number of Individual Case Studies for this Cause Variable.
                [Formula: 
                    update variables
                        left join (
                            select count(id) as total, cause_variable_id
                ...
                ');
            $table->integer('number_of_measurements')->nullable()->comment('Number of Measurements for this Variable.
            ...
            $table->integer('number_of_predictor_case_studies')->nullable()->comment('Number of Individual Case Studies for this Effect Variable.
                    [Formula: update variables
                        left join (
                            select count(id) as total, effect_variable_id
                            from user_variable_relationships
            ...
            $table->integer('number_of_studies_where_cause_variable')->nullable()->comment('Number of Studies for this Cause Variable.
                    [Formula: update variables
                        left join (
                            select count(id) as total, cause_variable_id
                            from studies
            ...
            $table->integer('number_of_studies_where_effect_variable')->nullable()->comment('Number of Studies for this Effect Variable.
                    [Formula: update variables
                        left join (
                            select count(id) as total, effect_variable_id
                            from studies
            ...
            $table->integer('number_of_tracking_reminder_notifications')->nullable()->comment('Number of Tracking Reminder Notifications for this Variable.
                    [Formula: update variables
                        left join (
                            select count(id) as total, variable_id
            ...
            $table->integer('number_of_user_tags_where_tag_variable')->nullable()->comment('Number of User Tags for this Tag Variable.
                    [Formula: update variables
                        left join (
                            select count(id) as total, tag_variable_id
                            from user_tags
            ...
            $table->integer('number_of_user_tags_where_tagged_variable')->nullable()->comment('Number of User Tags for this Tagged Variable.
                    [Formula: update variables
                        left join (
                            select count(id) as total, tagged_variable_id
                            from user_tags
            ...
            $table->integer('number_of_variables_where_best_cause_variable')->nullable()->comment('Number of Variables for this Best Cause Variable.
                    [Formula: update variables
                        left join (
            ...
            $table->integer('number_of_variables_where_best_effect_variable')->nullable()->comment('Number of Variables for this Best Effect Variable.
                    [Formula: update variables
                        left join (
            ...
            $table->integer('number_of_votes_where_cause_variable')->nullable()->comment('Number of Votes for this Cause Variable.
                    [Formula: update variables
                        left join (
                            select count(id) as total, cause_variable_id
                            from votes
            ...
            $table->integer('number_of_votes_where_effect_variable')->nullable()->comment('Number of Votes for this Effect Variable.
                    [Formula: update variables
                        left join (
                            select count(id) as total, effect_variable_id
                            from votes
            ...
            $table->integer('number_of_users_where_primary_outcome_variable')->nullable()->comment('Number of Users for this Primary Outcome Variable.
                    [Formula: update variables
                        left join (
            ...
            $table->float('maximum_allowed_daily_value', 0, 0)->nullable()->comment('The maximum allowed value in the default unit for measurements aggregated over a single day. ');
            ...
            $table->string('slug', 200)->nullable()->unique('variables_slug_uindex')->comment('The slug is the part of a URL that identifies a page in human-readable keywords.');
	         $table->enum('is_goal', ['ALWAYS', 'SOMETIMES', 'NEVER', NULL])
		         ->nullable()->comment('The effect of a food on the severity of a symptom is useful because you can control the predictor directly. However, the effect of a symptom on the foods you eat is not very useful.  The foods you eat are not generally an objective end in themselves. ');
            ...
            $table->integer('canonical_variable_id')->nullable()->comment('If a variable duplicates another but with a different name, set the canonical variable id to match the variable with the more appropriate name.  Then only the canonical variable will be displayed and all data for the duplicate variable will be included when fetching data for the canonical variable. ');
