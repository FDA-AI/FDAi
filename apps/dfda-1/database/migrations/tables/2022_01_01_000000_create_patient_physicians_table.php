...
class CreatePatientPhysiciansTable extends Migration
{
    ...
    public function up()
    {
         Schema::create('patient_physicians', function (Blueprint $table) {
            $table->increments('id');
            $table->bigInteger('patient_user_id')->comment('The patient who has granted data access to the physician. ');
            $table->bigInteger('physician_user_id')->index('patient_physicians_wp_users_ID_fk_2')->comment('The physician who has been granted access to the patients data.');
