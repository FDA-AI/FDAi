create table quantimodo_test.variables
(
    ...
    constraint variables_best_effect_variable_id_fk
        foreign key (best_effect_variable_id) references quantimodo_test.variables (id)
            on delete set null,
...
create index IDX_cat_unit_public_name
    on quantimodo_test.variables (variable_category_id, default_unit_id, name, number_of_user_variables, id);

create index fk_variableDefaultUnit
    on quantimodo_test.variables (default_unit_id);

create index public_deleted_at_synonyms_number_of_user_variables_index
    on quantimodo_test.variables (deleted_at, synonyms, number_of_user_variables);

create index variables_analysis_ended_at_index
    on quantimodo_test.variables (analysis_ended_at);

create index variables_public_name_number_of_user_variables_index
    on quantimodo_test.variables (name, number_of_user_variables);
