create table quantimodo_test.user_variables
(
    ...
    best_cause_variable_id                               int                                      null,
    best_effect_variable_id                              int                                      null,
    ...
    constraint user_variables_correlations_qm_score_fk
        foreign key (best_user_correlation_id) references quantimodo_test.correlations (id)
            on delete set null,
    constraint user_variables_default_unit_id_fk
        foreign key (default_unit_id) references quantimodo_test.units (id),
    constraint user_variables_last_unit_id_fk
        foreign key (last_unit_id) references quantimodo_test.units (id),
...
create index fk_variableSettings
    on quantimodo_test.user_variables (variable_id);

create index user_variables_analysis_started_at_index
    on quantimodo_test.user_variables (analysis_started_at);

create index user_variables_user_id_latest_tagged_measurement_time_index
    on quantimodo_test.user_variables (user_id);

create index variables_analysis_ended_at_index
    on quantimodo_test.user_variables (analysis_ended_at);
