create table quantimodo_test.tracking_reminders
(
    ...
    reminder_start_time                             time      default '00:00:00'        not null comment 'LOCAL TIME: Earliest time of day at which reminders should appear',
    reminder_end_time                               time                                null comment 'LOCAL TIME: Latest time of day at which reminders should appear',
    ...
    constraint tracking_reminders_user_variables_user_id_variable_id_fk
        foreign key (user_id, variable_id) references quantimodo_test.user_variables (user_id, variable_id),
...
create index tracking_reminders_user_variables_variable_id_user_id_fk
    on quantimodo_test.tracking_reminders (variable_id, user_id);

create index user_client
    on quantimodo_test.tracking_reminders (user_id, client_id);
