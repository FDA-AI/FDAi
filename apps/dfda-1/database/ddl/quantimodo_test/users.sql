create table quantimodo_test.users
(
    id                                                       bigint unsigned auto_increment comment 'Unique number assigned to each user.'
        primary key,
    client_id                                                varchar(255)                           not null,
    name                                                     varchar(60)                            null comment 'Unique username for the user.',
    email                                                    varchar(320)                           null comment 'Email address of the user.',
    password                                                 varchar(255)                           null comment 'Hash of the user’s password.',
    user_url                                                 varchar(100) default ''                not null,
    user_activation_key                                      varchar(255)                           null comment 'Used for resetting passwords.',
    display_name                                             varchar(250)                           null comment 'Desired name to be used publicly in the site, can be name, first name or last name defined in wp_usermeta.',
    ...
    earliest_reminder_time                                   time         default '07:00:00'        not null comment 'Earliest time of day at which reminders should appear in HH:MM:SS format in user timezone',
    latest_reminder_time                                     time         default '21:00:00'        not null comment 'Latest time of day at which reminders should appear in HH:MM:SS format in user timezone',
    ...
    number_of_users_where_referrer_user                      int unsigned                           null,
    ...
    constraint users_users_id_fk
        foreign key (referrer_user_id) references quantimodo_test.users (id),
    constraint users_variables_id_fk
        foreign key (primary_outcome_variable_id) references quantimodo_test.variables (id)
)
    comment 'General user information and overall statistics' charset = utf8mb3;

create index display_name
    on quantimodo_test.users (display_name);

create index users_stripe_id_index
    on quantimodo_test.users (stripe_id);
