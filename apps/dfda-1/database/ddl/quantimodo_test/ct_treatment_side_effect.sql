create table quantimodo_test.ct_treatment_side_effect
(
    ...
    constraint side_effect_variables_id_fk
        foreign key (side_effect_variable_id) references quantimodo_test.variables (id),
    constraint treatment_side_effect_side_effects_id_fk
        foreign key (side_effect_id) references quantimodo_test.ct_side_effects (id),
    constraint treatment_side_effect_treatments_id_fk
        foreign key (treatment_id) references quantimodo_test.ct_treatments (id),
    constraint treatment_variables_id_fk
        foreign key (treatment_variable_id) references quantimodo_test.variables (id)
)
    charset = utf8mb3;
