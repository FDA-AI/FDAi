create table quantimodo_test.third_party_correlations
(
    ...
    predicts_low_effect_change                                   int                                 null comment 'The percent change in the outcome from average typically seen when the predictor value is closer to the predictsHighEffect value.',
    ...
    constraint third_party_correlations_cause_variables_id_fk
        foreign key (cause_id) references quantimodo_test.variables (id),
    ...
    constraint third_party_correlations_effect_variables_id_fk
        foreign key (effect_id) references quantimodo_test.variables (id)
)
    comment 'Stores Calculated Correlation Coefficients' charset = utf8mb3;

create index cause
    on quantimodo_test.third_party_correlations (cause_id);

create index effect
    on quantimodo_test.third_party_correlations (effect_id);
