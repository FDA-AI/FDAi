create table quantimodo_test.correlations
(
    ...
    predicts_high_effect_change                                  int                                                             null comment 'The percent change in the outcome typically seen when the predictor value is closer to the predictsHighEffect value. ',
    ...
    onset_delay_with_strongest_pearson_correlation               int                                                             null,
...
create index correlations_analysis_started_at_index
    on quantimodo_test.correlations (analysis_started_at);

create index correlations_deleted_at_analysis_ended_at_index
    on quantimodo_test.correlations (deleted_at, analysis_ended_at);

create index correlations_updated_at_index
    on quantimodo_test.correlations (updated_at);

create index correlations_user_id_deleted_at_qm_score_index
    on quantimodo_test.correlations (user_id, deleted_at, qm_score);

create index user_id_cause_variable_id_deleted_at_qm_score_index
    on quantimodo_test.correlations (user_id, cause_variable_id, deleted_at, qm_score);

create index user_id_effect_variable_id_deleted_at_qm_score_index
    on quantimodo_test.correlations (user_id, effect_variable_id, deleted_at, qm_score);
