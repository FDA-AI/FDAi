<?php
/*
*  GNU General Public License v3.0
*  Contributors: ADD YOUR NAME HERE, Mike P. Sinn
 */

namespace App\Buttons\RelationshipButtons\ButtonClick;
use App\Buttons\RelationshipButtons\BelongsToRelationshipButton;
use App\Models\ButtonClick;
use App\Models\User;
class ButtonClickUserButton extends BelongsToRelationshipButton {
	public $interesting = true;
	public $foreignKeyName = ButtonClick::FIELD_USER_ID;
	public $qualifiedForeignKeyName = ButtonClick::TABLE . '.' . ButtonClick::FIELD_USER_ID;
	public $ownerKeyName = User::FIELD_ID;
	public $qualifiedOwnerKeyName = User::TABLE . '.' . User::FIELD_ID;
	public $childClass = ButtonClick::class;
	public $parentClass = ButtonClick::class;
	public $qualifiedParentKeyName = ButtonClick::TABLE . '.' . ButtonClick::FIELD_ID;
	public $relatedClass = User::class;
	public $methodName = 'user';
	public $relationshipType = 'Illuminate\\Database\\Eloquent\\Relations\\BelongsTo';
	public $color = User::COLOR;
	public $fontAwesome = User::FONT_AWESOME;
	public $id = 'user-button';
	public $image = ButtonClick::DEFAULT_IMAGE;
	public $text = 'User';
	public $title = 'User';
	public $tooltip = User::CLASS_DESCRIPTION;
}
