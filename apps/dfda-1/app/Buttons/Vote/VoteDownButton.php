...
use App\UI\IonIcon;
class VoteDownButton extends QMButton {
	...
	public function __construct(string $url, array $params, string $causeVariableName, string $effectVariableName,
		string $text){
		$params['vote'] = "down";
		parent::__construct($text, null, null, IonIcon::ion_icon_thumbs_down);
		$this->setWebhookUrl($url);
		$this->setFunctionName("vote");
		$this->setParameters($params);
		$message = "seems unlikely that " . $causeVariableName . " could influence " . $effectVariableName;
		$this->setConfirmationText("Do you agree that it $message?");
		$this->setSuccessToastText("Thank you for teaching me!");
		$this->setAdditionalInformationAndTooltip("Down Vote");
	}
}
