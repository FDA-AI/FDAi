 ...
 */
class AccessTokenCleanUpJobTest extends JobTestCase {
    public function testFixAccessTokensWithInvalidClientIds(){
        self::deleteGhostInspectorTokens();
        self::changeClientIdToQuantiModoWhereClientIdIsGoogle();
        self::changeClientIdToQuantiModoWhereClientIdIsMozilla();
        self::changeClientIdToQuantiModoWhereClientIsChromeExtension();
        self::deleteAccessTokensWithPeriodsInClientId();
    }
    private static function deleteGhostInspectorTokens(){
        $qb = QMAccessToken::readonly()
            ->whereLike(OAAccessToken::FIELD_CLIENT_ID,  "%Ghost%");
        $rows = $qb->getArray();
        if($rows){
    ...
    private static function deleteAccessTokensWithPeriodsInClientId(){
        $qb = QMAccessToken::readonly()->whereLike(OAAccessToken::FIELD_CLIENT_ID, "%.%");
    ...
    private static function changeClientIdToQuantiModoWhereClientIdIsMozilla(){
        $qb = QMAccessToken::readonly()
            ->whereLike(OAAccessToken::FIELD_CLIENT_ID, '%Mozilla%');
        $rows = $qb->getArray();
        if($rows){
            QMLog::error(count($rows) . " " . __FUNCTION__ . " to update");
            $qb->update([QMClient::FIELD_CLIENT_ID => BaseClientIdProperty::CLIENT_ID_QUANTIMODO]);
