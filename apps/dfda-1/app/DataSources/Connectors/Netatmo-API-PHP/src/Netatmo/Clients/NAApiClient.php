...
class NAApiClient {
     ...
     * @return mixed The json_decoded result or NAClientException if pb happend
     ...
     * @return array|mixed A valid array containing at least an access_token as an index
        ...
        if($this->access_token){
            if(!is_null($this->expires_at) && $this->expires_at < time()){
                if($this->refresh_token){
                    return $this->getAccessTokenFromRefreshToken();//exception will be thrown otherwise
                }
     ...
     *   endpoint.
     * @return mixed A valid OAuth2.0 JSON decoded access token in associative array
    ...
    private function getAccessTokenFromAuthorizationCode($code){
        $scope = $this->getVariable('scope');
        if($this->getVariable('access_token_uri') && $this->getRedirectUri() != NULL){
            $ret = $this->makeRequest($this->getVariable('access_token_uri'), 'POST', [
    ...
    private function getClientId(){
        return NetatmoConnector::instance()->getConnectorClientId();
    }
    /**
     * @return string
     */
    private function getClientSecret(){
        return NetatmoConnector::getClientSecret();
    }
    /**
     * Get access token from OAuth2.0 token endpoint with basic user
     * credentials.
    ...
    private function getAccessTokenFromRefreshToken(){
        if($this->getVariable('access_token_uri') && ($refresh_token = $this->refresh_token) != NULL){
     ...
     */
    protected function makeOAuth2Request($path, $method = 'GET', $params = [], $reget_token = true){
     ...
     *   (optional The GET/POST parameters.
     * @param bool $secure
     * @return array|mixed The JSON decoded body response object.
     ...
     * @throws NANotLoggedErrorType
     */
    public function api($path, $method = 'GET', $params = [], $secure = false){
        ...
        $res = $this->makeOAuth2Request($this->getUri($path, [], $secure), $method, $params);
        return $res["body"] ?? $res;
    }
    /**
     ...
     * @param array $params
     *   (optional The GET/POST parameters.
     * @return mixed The JSON decoded response object.
     *   The JSON decoded response object.
     * @throws NAApiErrorType
     * @throws NACurlErrorType
     * @throws NAJsonErrorType
     */
    public function noTokenApi($path, $method = 'GET', $params = []){
        if(is_array($method) && empty($params)){
            $params = $method;
            $method = 'GET';
        }
    ...
    /**
     * Returns the Current URL.
     * @return mixed|string|null The current URL.
     *   The current URL.
     */
     ...
     * @param bool $secure
     * @return mixed|string|null The URL for the given parameters.
     *   The URL for the given parameters.
     */
    public function getUri($path = '', $params = [], $secure = false){
...
class NAApiHelper {
     ...
     * @return null
     */
    public function api($method, $action, $params = []){
    ...
    public function simplifyDeviceList($app_type = "app_station"){
        $this->devices = $this->client->api("devicelist", "POST", ["app_type" => $app_type]);
