...
class NAEvent extends NAObjectWithPicture {
    private static $videoEvents = [
        NACameraEventType::CET_PERSON,
        NACameraEventType::CET_MOVEMENT
    ];
    /**
    ...
    public function getEventSubType(){
        if($this->getEventType() === NACameraEventType::CET_SD || $this->getEventType() === NACameraEventType::CET_ALIM){
            return $this->getVar(NACameraEventInfo::CEI_SUB_TYPE);
        }else{
            throw new NASDKException(NASDKErrorCode::INVALID_FIELD, "This field does not exist for this type of event");
        }
    }
    /**
     * @return string
    ...
    public function getPersonId(){
        if($this->getEventType() === NACameraEventType::CET_PERSON || $this->getEventType() === NACameraEventType::CET_PERSON_AWAY){
    ...
    public function hasVideo(){
        if(in_array($this->getEventType(), $this->videoEvents, true)){
            return TRUE;
        }else{
            return FALSE;
        }
    ...
    public function getVideo(){
        if($this->hasVideo()){
            ...
            throw new NASDKException(NASDKErrorCode::INVALID_FIELD, "This type of event does not have videos");
        }
    }
    /**
     * @return string
