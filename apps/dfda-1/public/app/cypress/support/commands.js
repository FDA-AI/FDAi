...
Cypress.Commands.add('goToLoginPageAndLogin', function (email, password) {
    cy.log("=== goToLoginPageAndLogin as " + email + " ===");
    cy.visitIonicAndSetApiOrigin('/#/app/login?logout=1')
    cy.log('Clicking #email-login-button');
    ...
    cy.get('#emailLogin').click();
    cy.log('Clicking #emailLogin');
});
...
Cypress.Commands.add('enterNewUserCredentials', function (username, email, password) {
    ...
    cy.get('input[name="user_login"]').type(username, { force: true });
    cy.get('input[name="email"]').type(email, { force: true });
    cy.get('input[name="password"]').click({ force: true }).type(password, { force: true });
    cy.get('input[name="passwordConfirm"]').click({ force: true }).type(password, { force: true });
...
Cypress.Commands.add('checkForBrokenImages', function () {
    ...
    cy.get('img', { timeout: 30000 })
        ...
        var src_1 = $el[0].getAttribute('src');
            var message = "The image with public \n  " + src_1 +
                          " \n  is broken! \n outerHTML is: \n  " +
                          $el[0].outerHTML
            //cy.debug("Checking image at index: " + index +" with public "+src_1, $el);
