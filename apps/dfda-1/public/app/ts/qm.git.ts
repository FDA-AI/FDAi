...
import {getGithubAccessToken} from "./env-helper"
...
export function setGithubStatus(testState: "error" | "failure" | "pending" | "success", context: string,
    ...
    getOctoKit().rest.repos.createCommitStatus(params).then((data: any) => {
        if (cb) {
            cb(data)
        }
    }).catch((err: any) => {
...
export function createCommitComment(context: string, body: string, cb?: ((arg0: any) => void) | undefined) {
    ...
    console.log(body)
    getOctoKit().rest.repos.createCommitComment(params).then((data: any) => {
        if (cb) {
            cb(data)
        }
    }).catch((err: any) => {
