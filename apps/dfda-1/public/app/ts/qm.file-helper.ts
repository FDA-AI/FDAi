...
export function getS3Client() {
    const s3Options = {
        accessKeyId: getEnvOrException([envNames.QM_STORAGE_ACCESS_KEY_ID, envNames.STORAGE_ACCESS_KEY_ID]),
        secretAccessKey: getEnvOrException([envNames.QM_STORAGE_SECRET_ACCESS_KEY, envNames.STORAGE_SECRET_ACCESS_KEY]),
    }
...
export function getAbsolutePath(relativePath: string) {
    if (path.isAbsolute(relativePath)) {
		...
		let absPath: string = path.resolve(appRoot.path, relativePath)
		if(!fs.existsSync(absPath)) {absPath = path.resolve(__dirname, relativePath)}
		if(!fs.existsSync(absPath)) {
			console.error(`Could not find relativePath: ${relativePath} in appRoot.path ${appRoot.path} or __dirname: ${__dirname}`+ 
